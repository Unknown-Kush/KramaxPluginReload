...
namespace KramaxPluginReload
{
    ...
    */
    ...
    public class PluginReloadModule : MonoBehaviour
    {
        ...
        private static Assembly LoadAssembly(string location, int versionCount)
        {
            try
            {
                ...
                //The bug is corrected in mono-6.6.0.161 and mono-5.16.0.179 but mono used in KSP 1.8.1 is
                //reported as "5.11.0 (Visual Studio built mono)". To hack around this we do the following:
                // - Decompile .dll using ildasm
                // - Change assembly name inside decomiled file (simple regex, fragment ".assembly <assembly-name>")
                ...
                // String tmpLocation = location + "-tmp-" + versionCount + ".dll";
                //System.IO.File.Copy(location, tmpLocation);
                //Assembly a = Assembly.Load(tmpLocation);
                ...
                if (type != null)
                {
                    ...
                    {
                        Deb.Log("Mono version: {0}", dispalayName.Invoke(null, null));
                    }
                }
                Deb.Log(
                    "KramaxPluginReload: Reloaded assembly: {0} version: {1}, location: {2}, md5: {3}.", 
                    a.GetName().Name, 
                    a.GetName().Version,                   
                    location, 
                    GetMd5Hash(assemblyBytes)
                    );
                a.GetName().Name = a.GetName().Name + "-" + versionCount;
                return a;
            }
            catch (Exception ex)
            {
        ...
        static Type CreateUniqueSubClass(ModuleBuilder moduleBldr, Type originalType, int versionUid, KSPAddon kspAddon)
        {
            ...
            if (kspAddon != null) { 
                CustomAttributeBuilder customAttributeBuilder =
                    new CustomAttributeBuilder(
                        typeof(KSPAddon).GetConstructor(new Type[] { typeof(KSPAddon.Startup), typeof(bool) }),
                        new object[] { kspAddon.startup, kspAddon.once }
                       );
                typeBldr.SetCustomAttribute(customAttributeBuilder);
            }

            ConstructorBuilder ctor = typeBldr.DefineDefaultConstructor(MethodAttributes.Public);            
            CreateMethodCallingSuper(typeBldr, originalType, "Start", MethodAttributes.Public | MethodAttributes.Virtual, typeof(void), new Type[0]);
            CreateMethodCallingSuper(typeBldr, originalType, "OnDestroy", MethodAttributes.Public | MethodAttributes.Virtual, typeof(void), new Type[0]);
            return typeBldr.CreateType();
        }


        static void CreateMethodCallingSuper(
            TypeBuilder typeBldr,
            Type originalType,
            String methodName,
            ...
            Type[] typeArguments)
        {
            ...
            MethodInfo baseMethod = originalType.GetMethod(methodName, typeArguments);
            if (baseMethod == null)
            {
                Deb.Log("Method {0} does not exist in {1}, will not be supported", methodName, originalType);
                return;
            }
            else
            {
                Deb.Log("Method {0} exists in {1}", methodName, originalType);
            }
            MethodBuilder startBuilder = typeBldr.DefineMethod(methodName, attributes, returntype, typeArguments);
            ILGenerator ilGenerator = startBuilder.GetILGenerator();
            ...
            {
                for (int i = 4; i <= typeArguments.Length; ++i)
                {
        ...
        private static void LoadPlugins()
        {
            ...
            foreach (PluginSetting setting in PluginSettings)
            {
                ...
                var assembly = LoadAssembly(setting.Path, versionCount);
                ...
                //Deb.Log("KramaxPluginReload: plugin: {0}, tmpAssemblyName: {1}, tmpModuleName: {2}.", setting.Name, tmpAssemblyName, tmpModuleName);
                ...
                //        pluginClass.type = CreateUniqueSubClass(moduleBldr, derivedClass, versionCount, kspAddon);
                ...
                //        var newType = CreateUniqueSubClass(moduleBldr, derivedClass, versionCount, kspAddon);
